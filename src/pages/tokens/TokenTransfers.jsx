...
import { ChevronLeft, ChevronRight, Copy, FileText, Eye, Download, ArrowRight, RefreshCw, Coins, AlertCircle } from 'lucide-react'
import veroService from '../../services/veroService'

const TokenTransfers = () => {
  const [currentPage, setCurrentPage] = useState(1)
  const [rowsPerPage, setRowsPerPage] = useState(50)
  const [showAmount, setShowAmount] = useState(true) // true = Amount, false = Value (USD)
  ...
  const fetchTokenTransfers = async () => {
    try {
      ...
      const transfers = await veroService.getTokenTransfers(rowsPerPage)
  ...
  const downloadPageData = () => {
    ...
    a.href = url
    a.download = `token-transfers-page-${currentPage}.json`
    a.click()
      ...
      <div className="bg-white dark:bg-black border-b border-gray-200 dark:border-gray-800 py-4">
        <div className="container max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Token Transfers (ERC-20)</h1>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                {loading ? 'Loading token transfers...' : 
                 error ? 'Failed to load token transfers' : 
                 tokenTransfers.length === 0 ? 'No token transfers found' :
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-blue-900 dark:text-blue-200">
              <p className="font-medium mb-1">About ERC-20 Token Transfers</p>
              <p className="text-blue-700 dark:text-blue-300">
                This page shows ERC-20 token transfers on the Vero Network. These are transfers of smart contract tokens, not the native VERO token.
              </p>
            </div>
          </div>
        </div>
                ...
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                    <span className="flex items-center gap-1">
                      Method
                      <i className="text-gray-400 text-xs" title="Function executed based on decoded input data">?</i>
                    </span>
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                    Block
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                    <button
                      ...
                      title="Click to toggle between Amount and Value (USD)"
                    >
                      {showAmount ? 'Amount' : 'Value (USD)'}
                      {!showAmount && <i className="text-gray-400 text-xs" title="Value displayed based on latest available exchange rate">?</i>}
                    </button>
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                    Token
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="10" className="px-4 py-8 text-center">
                      <div className="text-red-600 dark:text-red-400">
                        <p className="font-medium">Failed to load token transfers</p>
                  ...
                  <tr>
                    <td colSpan="10" className="px-4 py-8 text-center">
                      <div className="text-gray-500 dark:text-gray-400">
                        ...
                        <p className="font-medium">No ERC-20 token transfers found</p>
                        <p className="text-sm mt-1">No ERC-20 token transfers found in recent transactions</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  tokenTransfers.map((transfer, index) => (
                    <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-block px-2 py-1 text-xs font-medium bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded border border-green-200 dark:border-green-800">
                          {transfer.method}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="text-sm">
                          <div className="text-gray-900 dark:text-gray-100 font-medium">
                            {showAmount ? `${transfer.amount} ${transfer.tokenSymbol}` : transfer.valueUSD}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
